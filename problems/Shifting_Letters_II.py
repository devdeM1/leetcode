class Solution(object):
    def shiftingLetters(self, s, shifts):
        ...
        # def change_symbols(str_before, direction, alphabet):
        #     str_after = ''
        #     if direction:
        #         for symbol in str_before:
        #             if symbol != 'z':
        #                 str_after += alphabet[alphabet.index(symbol) + 1]
        #             else:
        #                 str_after += 'a'
        #     else:
        #         for symbol in str_before:
        ...
        #     sub_str_before = s[left:right + 1]
        #     s = s.replace(sub_str_before, change_symbols(str_before = sub_str_before,
        #                                                  direction = direction,
        ...
        # my_list = list(s)
        # alphabet = 'abcdefghijklmnopqrstuvwxyz'
        # diff_list = [0] * len(my_list)
        #
        # for left, right, direction in shifts:
        #     for i in range(left, right + 1):
        #         if direction:
        #             diff_list[i] += direction
        #         else:
        #             diff_list[i] += -1
        #
        # for i in range(len(my_list)):
        #     diff_list[i] = diff_list[i] % len(alphabet)
        #     try:
        #         my_list[i] = alphabet[alphabet.index(my_list[i]) + diff_list[i]]
        #     except IndexError:
        #         if diff_list[i] > 0:
        #             my_list[i] = alphabet[abs(len(alphabet) - diff_list[i] - alphabet.index(my_list[i]))]
        #         else:
        #             my_list[i] = alphabet[abs(len(alphabet) - diff_list[i] + alphabet.index(my_list[i]))]
        # return ''.join(my_list)
        #
        ...
        for left, right, direction in shifts:
            if direction:
                diff_list[left] += direction
                if right!= len(my_list) - 1:
                    diff_list[right + 1] -= direction
            else:
                diff_list[left] -= 1
                if right!= len(my_list) - 1:
                    diff_list[right + 1] += 1
        ...
        for i in range(len(my_list)):
            change += diff_list[i]
            change = change % len(alphabet)
            try:
                my_list[i] = alphabet[alphabet.index(my_list[i]) + change]
            except IndexError:
                if change > 0:
                    try:
                        my_list[i] = alphabet[abs(len(alphabet) - change - alphabet.index(my_list[i]))]
                    except:
                        print('asd')
                else:
                    my_list[i] = alphabet[abs(len(alphabet) - change + alphabet.index(my_list[i]))]
